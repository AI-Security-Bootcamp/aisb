...
def complete_with_prefill(
    ...
    """
    # Ending on an assistant message leaves the turn open, so the model
    # continues the prefill instead of starting a fresh reply.
    ...
    # The API returns only the continuation, so re-attach the prefill to get
    # the assistant turn as the model "sees" it. content is Optional.

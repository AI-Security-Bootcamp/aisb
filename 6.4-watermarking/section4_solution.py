...
does that compare with a post-hoc watermark whose extractor is a small standalone network?</summary>
...
breaks with this scheme, and what would you change first?</summary>
...
def ring_flatness(latent: Float[Tensor, "1 4 h w"], radius: int = RADIUS, channel: int = CHANNEL) -> float:
    ...
    if "SOLUTION":
        spec = to_spectrum(latent[0, channel].float()).cpu()
        d = radial_distance()
        return float(np.mean([spec[(d >= i - 1) & (d < i)].std().item() for i in range(2, radius + 1)]))
    else:

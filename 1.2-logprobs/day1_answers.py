...
def get_completion_with_logprobs(
    ...
    # The API returns one entry in `choices` per requested completion; we asked
    # for the default n=1, so the single completion we want is choices[0].
    choice = response.choices[0]
    if choice.logprobs is None or choice.logprobs.content is None:
        raise ValueError(f"{model} returned no logprobs; it may not support them")

    # logprobs.content has one entry per generated token. Each entry carries
    # the alternatives the model weighed at that position, best-first.
    return [
        [(alt.token, alt.logprob) for alt in (position.top_logprobs or [])]

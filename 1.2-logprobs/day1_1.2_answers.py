...
def get_completion_with_logprobs(
    ...
    logprob_content = res.choices[0].logprobs.content

    for lg in logprob_content:
        top_lg_result = []

        for top_lg in lg.top_logprobs:
            top_lg_result.append((top_lg.token, top_lg.logprob))

        result.append(top_lg_result)
